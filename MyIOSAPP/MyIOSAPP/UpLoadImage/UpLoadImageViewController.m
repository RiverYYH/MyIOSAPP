...
@interface UpLoadImageViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,JJPhotoDelegate,HWImagePickerSheetDelegate>{
...
-(void)initPickerView{
    ...
    [self.showInView setBackgroundColor:[UIColor blueColor]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ((indexPath.row == _imageArray.count) && (_imageArray.count < 4)) {
        ...
        if (indexPath.item < _imageArray.count) {
            ...
            [cell.profilePhoto setImage:nil];
            cell.closeButton.hidden = YES;
        }
    }
...
{
    return CGSizeMake(([UIScreen mainScreen].bounds.size.width-64) /5 ,([UIScreen mainScreen].bounds.size.width-64) /5);
}
...
- (void)deletePhoto:(UIButton *)sender{
    ...
    //    }
    [self.pickerCollectionView reloadSections:[NSIndexSet indexSetWithIndex:0]];
    [self changeCollectionViewHeight];
}
...
- (void)changeCollectionViewHeight{
    ...
    //    }
    //    else{
    //        _pickerCollectionView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, (((float)[UIScreen mainScreen].bounds.size.width-64.0) /5.0 +20.0)* ((int)(_arrSelected.count)/5 +1)+20.0);
    //    }
    _pickerCollectionView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width,[UIScreen mainScreen].bounds.size.height -200 );
