...
@interface GitSelectionHolderView()
...
@end
@implementation GitSelectionHolderView
...
- (void)buttonSendClicked:(id)sender
{
    ...
    if (_selectedItem) {
        if(self.delegate && [self.delegate respondsToSelector:@selector(sendGiftWithIndex:)]){
            [self.delegate sendGiftWithIndex:(int)_selectedItem.tag];
        }
    }
}
