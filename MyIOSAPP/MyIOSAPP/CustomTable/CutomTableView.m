...
@interface CutomTableView()<UITableViewDelegate,UITableViewDataSource>
{
...
- (void)drawCell:(CustomTableViewCell *)cell withIndexPath:(NSIndexPath *)indexPath{
...
- (void)loadContent{
    ...
    if (self.visibleCells&&self.visibleCells.count>0) {
        for (id temp in [self.visibleCells copy]) {
            CustomTableViewCell *cell = (CustomTableViewCell *)temp;
            [cell draw];
        }
    }
}
...
- (void)removeFromSuperview{
    for (UIView *temp in self.subviews) {
        for (CustomTableViewCell *cell in temp.subviews) {
